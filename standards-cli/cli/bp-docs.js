...
program
  ...
  .option(
    '--maintenance',
    'Generate maintenance files (dependencies.md, sdk-map.md)',
    false
  )
  .parse(process.argv);
...
function extractReturnType(lines, functionLine) {
  ...
  for (
    let i = functionLine;
    i < Math.min(lines.length, functionLine + 50);
    i++
  ) {
    const line = lines[i].trim();
...
function extractExamples(_lines, _functionLine) {
  const examples = [];
...
${
  func.parameters.length > 0
    ? func.parameters
        .map(p => `- \`${p.name}\`${p.optional ? ' (optional)' : ''}`)
        .join('\n')
    : 'None'
}
...
async function generateDiagrams(docData, outputPath) {
  ...
  await fs.writeFile(
    path.join(outputPath, 'dependency-diagram.md'),
    dependencyDiagram
  );
}
...
${docData.modules
  .map(
    module =>
      `- [${module.name}](./modules/${module.name}.md) - ${module.description}`
  )
  .join('\n')}
...
async function generateMaintenanceFiles() {
  ...
  if (!(await fs.pathExists(dependenciesPath))) {
    ...
    console.log(
      chalk.yellow('    ⚠️  dependencies.md already exists, skipping')
    );
  }
