...
async function promptForConfiguration(options) {
  ...
  questions.push({
    ...
    choices: [
      {
        name: 'In-line comments for all functions',
        value: 'comments',
        checked: true,
      },
      {
        name: 'Maximum function length (50 lines)',
        value: 'function-length',
        checked: true,
      },
      {
        name: 'Security scanning (secrets, vulnerabilities)',
        value: 'security',
        checked: true,
      },
      {
        name: 'Performance monitoring (bundle size, speed)',
        value: 'performance',
        checked: true,
      },
      {
        name: '80% test coverage requirement',
        value: 'test-coverage',
        checked: true,
      },
    ],
  ...
  return {
    ...
    code: {
      ...
      maxFunctionLines:
        answers.standards?.includes('function-length') !== false ? 50 : null,
      testCoverage:
        answers.standards?.includes('test-coverage') !== false ? 80 : null,
    },
    security: {
    ...
    performance: {
      bundleSize:
        answers.standards?.includes('performance') !== false ? '500KB' : null,
      loadTime:
        answers.standards?.includes('performance') !== false ? '2s' : null,
    },
  };
}
