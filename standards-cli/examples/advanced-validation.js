...
async function advancedValidationExample() {
  ...
  try {
    ...
    console.log(
      `   Functions with comments: ${codeResults.metrics.commentedFunctions}/${codeResults.metrics.totalFunctions}`
    );
    ...
    console.log(
      `   Bundle size: ${performanceResults.metrics?.bundleSize || 'N/A'}`
    );
    ...
    const criticalIssues = comprehensiveResults.issues.filter(
      i => i.severity === 'error'
    );
    const warnings = comprehensiveResults.issues.filter(
