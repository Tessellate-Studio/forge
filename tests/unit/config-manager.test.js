...
describe('ConfigManager', () => {
  ...
  describe('getDefaultConfig', () => {
    test('should return valid default configuration', () => {
      ...
      expect(defaultConfig.profiles).toBeDefined();
      
      // Verify weights sum to 1.0
      const weightSum = Object.values(defaultConfig.weights).reduce((sum, weight) => sum + weight, 0);
    ...
    test('should include predefined profiles', () => {
      const defaultConfig = configManager.getDefaultConfig();
      
      expect(defaultConfig.profiles.startup).toBeDefined();
      expect(defaultConfig.profiles.enterprise).toBeDefined();
      expect(defaultConfig.profiles.research).toBeDefined();
    });
  });
  ...
  describe('validateConfig', () => {
    test('should validate correct configuration', () => {
      const validConfig = {
        weights: { impact: 0.35, complexity: 0.25, reusability: 0.20, strategic: 0.20 },
        thresholds: { high_priority: 3.5, medium_priority: 2.5, low_priority: 1.5 }
      };
    ...
    test('should detect missing weights', () => {
      ...
      expect(result.errors.some(e => e.includes('Missing weight: reusability'))).toBe(true);
    });
    
    test('should detect invalid weight range', () => {
      const invalidConfig = {
        weights: { impact: 1.5, complexity: 0.25, reusability: 0.20, strategic: 0.20 },
        thresholds: { high_priority: 3.5, medium_priority: 2.5, low_priority: 1.5 }
      };
      
      const result = configManager.validateConfig(invalidConfig);
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('Invalid weight for impact'))).toBe(true);
    });
    
    test('should detect incorrect weight sum', () => {
      const invalidConfig = {
        weights: { impact: 0.5, complexity: 0.3, reusability: 0.3, strategic: 0.2 }, // sums to 1.3
        thresholds: { high_priority: 3.5, medium_priority: 2.5, low_priority: 1.5 }
      };
    ...
    test('should detect invalid threshold order', () => {
      const invalidConfig = {
        weights: { impact: 0.35, complexity: 0.25, reusability: 0.20, strategic: 0.20 },
        thresholds: { high_priority: 2.0, medium_priority: 3.0, low_priority: 1.5 } // wrong order
      };
      
      const result = configManager.validateConfig(invalidConfig);
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('Thresholds must be in descending order'))).toBe(true);
    });
  });
  ...
  describe('initializeConfig', () => {
    ...
    test('should apply profile when specified', async () => {
      const configPath = path.join(tempDir, 'rubric-config.yml');
      ...
      // Verify profile weights were applied
      const configContent = await fs.readFile(configPath, 'utf8');
      expect(configContent).toContain('0.5'); // startup profile impact weight
    });
    
    test('should apply custom weights when specified', async () => {
      const customWeights = { impact: 0.4, complexity: 0.3, reusability: 0.2, strategic: 0.1 };
      const configPath = path.join(tempDir, 'rubric-config.yml');
    ...
    test('should not overwrite existing config without force flag', async () => {
      ...
      await expect(configManager.initializeConfig(tempDir)).rejects.toThrow('Configuration file already exists');
    });
    ...
    test('should overwrite existing config with force flag', async () => {
      ...
      const result = await configManager.initializeConfig(tempDir, { force: true });
  ...
  describe('getAvailableProfiles', () => {
    test('should return all available profiles', () => {
      ...
      expect(profileNames).toContain('startup');
      expect(profileNames).toContain('enterprise');
      expect(profileNames).toContain('research');
    });
    
    test('should include profile descriptions and weights', () => {
      const profiles = configManager.getAvailableProfiles();
