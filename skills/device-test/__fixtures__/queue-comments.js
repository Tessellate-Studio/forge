...
/**
 * THREE headings in one comment: a correction, then two whole tests. The
 * shape that motivated `itemHeadingCount` — captured before it was split on
 * 2026-09-09 into 5590806716 and 5590808504.
 *
 * On the board this showed as ONE row, titled after the correction, with
 * `budget-column-739`'s Status; `gender-unisex-739` fell below the first
 * rule after that Status and was filed as a NOTE, so it had no row at all.
 */
const stackedItems = alate(
  5589887980,
  [
    '### ⚠️ Correction to `fit-overlay-chrome-738` — the BUDGET expectations above are inverted',
    '',
    'I misread the original note. … [#739](https://github.com/Tessellate-Studio/alate/pull/739) flips it,',
    'so the steps in that item test the wrong thing.',
    '',
    '---',
    '',
    '### 🤖 budget-column-739 — BUDGET is always the fifth column, blank when no budget is set',
    '- **PR:** [#739](https://github.com/Tessellate-Studio/alate/pull/739)',
    '- **Delivery:** production OTA once #739 lands. Both platforms.',
    '- **Needs runtime:** 1.3.1',
    '- **Steps:**',
    '  1. HUMAN: Profile → Price range → **clear** any budget. Screenshot the stats row.',
    '  2. HUMAN: Profile → Price range → set 20–200. Screenshot the same row.',
    '- **Expect:**',
    '  1. Step 1: **five** columns, BUDGET an empty ring with a muted em dash.',
    '- **Status:** OPEN',
    '',
    '---',
    '',
    '### 🤖 gender-unisex-739 — the third gender chip stores `unisex`, not just displays it',
    '- **PR:** [#739](https://github.com/Tessellate-Studio/alate/pull/739)',
    '- **Delivery:** production OTA once #739 lands. Both platforms.',
    '- **Needs runtime:** 1.3.1',
    '- **Steps:**',
    '  1. HUMAN: Profile → Body profile → Edit → gender step. Pick **Unisex**.',
    '  2. HUMAN: force-quit, relaunch, return to the same step.',
    '- **Expect:**',
    '  1. The options read Woman / Man / Unisex, and Unisex survives the relaunch.',
    '- **Status:** OPEN',
    '',
    '---',
    '_Generated by [Claude Code](https://claude.ai/code)_',
