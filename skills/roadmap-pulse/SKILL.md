...
| **Label hygiene** | 0 or >1 P labels; no area label (loom, alate); an open PR closes it but no `claimed`; a closed issue still wearing `device-test`/`needs-*`; a label colour off `tools/labels/lib/labels.js` (→ `bootstrap.js`); `P0` with no activity for 14 days; `needs-input` > 14 days; `on hold` past its review-by date; a dead `claimed`; `needs-input` on a `decision` PR; a `decision` PR open > 14 days | Artifact section. `on hold` expiry → reminder comment. Dead `claimed` → `wip sweep` | Fix the labels with the owner |
