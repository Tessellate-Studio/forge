  ...
  .action(async options => {
    try {
      ...
      if (options.file) {
        ...
        console.log(
          chalk.blue(`📂 Loaded ${tasks.length} tasks from ${options.file}`)
        );
      }
          ...
          const { taskCount } = await inquirer.prompt([
            {
              ...
              validate: value =>
                (value >= 2 && value <= 10) ||
                'Please enter between 2 and 10 tasks',
            },
          ]);
      ...
      if (tasks.length < 2) {
        console.error(
          chalk.red('✖ Error: At least 2 tasks are required for comparison')
        );
        console.log(
          chalk.yellow(
            'Use --interactive flag to enter tasks or --file to load from JSON'
          )
        );
        process.exit(1);
      }
      ...
      for (const [index, task] of tasks.entries()) {
        if (!task.description) {
          console.error(
            chalk.red(`✖ Error: Task ${index + 1} missing description`)
          );
          process.exit(1);
        }
        if (!task.scores) {
...
function displayTableComparison(comparison) {
  ...
  console.log(
    `  High Priority: ${chalk.red(comparison.comparison.highPriority)}`
  );
  console.log(
    `  Medium Priority: ${chalk.yellow(comparison.comparison.mediumPriority)}`
  );
  console.log(
    `  Low Priority: ${chalk.blue(comparison.comparison.lowPriority)}`
  );
  ...
  comparison.evaluations.forEach((task, index) => {
    ...
    console.log(
      ...
      )} | Score: ${chalk.cyan(task.weightedScore)}`
    );
    console.log(
      `   Impact: ${task.scores.impact} | Complexity: ${task.scores.complexity} | Reusability: ${task.scores.reusability} | Strategic: ${task.scores.strategic}`
    );
  ...
  console.log(
    chalk.green(
      `Start with "${topTask.task}" (${topTask.priority} priority, score: ${topTask.weightedScore})`
    )
  );
}
