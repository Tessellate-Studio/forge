...
const initCommand = new Command('init')
  ...
  .option(
    '-p, --profile <name>',
    'Use a predefined profile (startup, enterprise, research)'
  )
  .option(
    '--weights <weights>',
  ...
  .action(async options => {
    try {
      ...
      if (options.interactive) {
        ...
        profileChoices.push({
          name: 'Custom - Configure weights manually',
          value: 'custom',
        });
        ...
        if (setup.profile !== 'custom') {
          ...
          console.log(
            chalk.gray(
              'Weights must sum to 1.0. Default values shown in parentheses.\n'
            )
          );
          ...
          const weightQuestions = [
            {
              ...
              default: 0.35,
              validate: value =>
                (value >= 0 && value <= 1) || 'Weight must be between 0 and 1',
            },
            {
              type: 'number',
              name: 'complexity',
          ...
          const weightSum = Object.values(customWeights).reduce(
          ...
          if (Math.abs(weightSum - 1.0) > 0.01) {
            console.error(
              chalk.red(
                `✖ Error: Weights must sum to 1.0, current sum: ${weightSum.toFixed(
                  2
                )}`
              )
            );
            console.log(
              chalk.yellow('Please restart and adjust your weights.')
            );
            process.exit(1);
          }
      ...
      if (result.success) {
        ...
        console.log(
          chalk.gray(
            '1. Run `rubric evaluate --interactive` to evaluate your first task'
          )
        );
        console.log(
          chalk.gray('2. Use `rubric compare` to compare multiple tasks')
        );
        ...
        console.log(
          chalk.gray(
            '  rubric evaluate "Add user authentication" --interactive'
          )
        );
        console.log(
          chalk.gray('  rubric report --format markdown --output report.md')
        );
        console.log(
          chalk.gray('  rubric config show  # View current configuration')
        );
      }
    } catch (error) {
