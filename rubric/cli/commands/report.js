...
const reportCommand = new Command('report')
  ...
  .option(
    '-p, --period <period>',
    'Time period (last-day, last-week, last-month, all)',
    'all'
  )
  .option(
    '--priority <level>',
    'Filter by priority level (high, medium, low, backlog)'
  )
  ...
  .action(async options => {
    try {
      ...
      if (options.input) {
        ...
        evaluations = Array.isArray(inputData)
          ? inputData
          : inputData.evaluations
          ? inputData.evaluations
          : [inputData];

        console.log(
          chalk.blue(
            `📂 Loaded ${evaluations.length} evaluations from ${options.input}`
          )
        );
      } else {
        ...
        if (evaluations.length === 0) {
          ...
          console.log(
            chalk.gray('Run some evaluations with --save flag to build history')
          );
          process.exit(0);
        }

        console.log(
          chalk.blue(`📊 Found ${evaluations.length} evaluations in history`)
        );
      }
      ...
      if (options.priority) {
        filteredEvaluations = filteredEvaluations.filter(
          e => e.priority === options.priority
        );
        console.log(
          chalk.gray(
            `🔍 Filtered to ${filteredEvaluations.length} ${options.priority} priority tasks`
          )
        );
      }
      ...
      if (options.evaluator) {
        filteredEvaluations = filteredEvaluations.filter(
          e => e.metadata?.evaluator === options.evaluator
        );
        console.log(
          chalk.gray(
            `🔍 Filtered to ${filteredEvaluations.length} evaluations by ${options.evaluator}`
          )
        );
      }
      ...
      if (filteredEvaluations.length === 0) {
        console.log(
          chalk.yellow('⚠️  No evaluations match the specified filters')
        );
        process.exit(0);
      }
      ...
      if (options.output) {
        ...
        console.log(
          chalk.gray(`📄 File size: ${(stats.size / 1024).toFixed(2)} KB`)
        );
      } else {
        // Output to console
...
async function loadEvaluationHistory(period) {
  ...
  for (const file of files) {
    try {
      ...
      console.warn(
        chalk.yellow(`Warning: Could not load ${file}: ${error.message}`)
      );
    }
  }
...
async function showSummaryStats(evaluations) {
  ...
  console.log(
    chalk.gray(
      `High Priority: ${priorities.high} (${(
        (priorities.high / evaluations.length) *
        100
      ).toFixed(1)}%)`
    )
  );
  console.log(
    chalk.gray(
      `Medium Priority: ${priorities.medium} (${(
        (priorities.medium / evaluations.length) *
        100
      ).toFixed(1)}%)`
    )
  );
  console.log(
    chalk.gray(
      `Low Priority: ${priorities.low} (${(
        (priorities.low / evaluations.length) *
        100
      ).toFixed(1)}%)`
    )
  );
  console.log(
    chalk.gray(
      `Backlog: ${priorities.backlog} (${(
        (priorities.backlog / evaluations.length) *
        100
      ).toFixed(1)}%)`
    )
  );
