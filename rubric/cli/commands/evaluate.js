...
const evaluateCommand = new Command('evaluate')
  .description(
    'Evaluate a single task or feature using the rubric scoring system'
  )
  .argument('[task]', 'Task description to evaluate')
  .option('-i, --interactive', 'Use interactive mode for scoring')
  .option(
    '-w, --weights <weights>',
    'Custom weights (e.g., impact=0.4,complexity=0.3)'
  )
    ...
    try {
      ...
      if (!taskDescription) {
        if (options.interactive) {
          ...
          console.log(
            chalk.yellow('Use --interactive flag or provide task as argument')
          );
          process.exit(1);
        }
      }
      ...
      if (options.interactive) {
        ...
        console.log(
          chalk.gray('Score each dimension from 0-5 (0=none, 5=maximum)\n')
        );
        ...
        const scoringQuestions = [
          {
            ...
            default: 3,
            validate: value =>
              (value >= 0 && value <= 5) || 'Score must be between 0 and 5',
          },
          {
            type: 'number',
            name: 'complexity',
          ...
          },
          {
            ...
            name: 'strategic',
            message:
              'Strategic Fit - How well does this align with vision/roadmap?',
            default: 3,
            validate: value =>
              (value >= 0 && value <= 5) || 'Score must be between 0 and 5',
          },
        ...
        console.error(
          chalk.red('✖ Error: Interactive mode is required for scoring')
        );
        console.log(
          chalk.yellow('Use --interactive flag to enter scores interactively')
        );
        console.log(
          chalk.gray('Future versions will support batch scoring from files')
        );
        process.exit(1);
      }
      ...
      if (options.weights) {
        try {
          ...
          console.log(
            chalk.yellow(
              'Use format: impact=0.4,complexity=0.3,reusability=0.2,strategic=0.1'
            )
          );
          process.exit(1);
        }
...
function displayTableOutput(evaluation) {
  ...
  console.log(
    chalk.bold('Priority:'),
    priorityColor(evaluation.priority.toUpperCase())
  );
  console.log(
    chalk.bold('Overall Score:'),
    chalk.cyan(evaluation.weightedScore)
  );

  console.log(chalk.bold('\nDimension Scores:'));
  console.log(
    `  Impact:        ${getScoreBar(evaluation.scores.impact)} ${
      evaluation.scores.impact
    }/5`
  );
  console.log(
    `  Complexity:    ${getScoreBar(evaluation.scores.complexity)} ${
      evaluation.scores.complexity
    }/5`
  );
  console.log(
    `  Reusability:   ${getScoreBar(evaluation.scores.reusability)} ${
      evaluation.scores.reusability
    }/5`
  );
  console.log(
    `  Strategic Fit: ${getScoreBar(evaluation.scores.strategic)} ${
      evaluation.scores.strategic
    }/5`
  );
