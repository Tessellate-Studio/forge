      ...
      Object.entries(config.thresholds).forEach(([key, value]) => {
        const color =
          key === 'high_priority'
            ...
            : chalk.gray;
        console.log(
          `  ${key.replace('_', ' ').padEnd(15)}: ${color(value.toFixed(1))}`
        );
      });
...
configCommand
  ...
  .action(() => {
    try {
      ...
      console.log(
        chalk.yellow('💡 Use `rubric init --profile <name>` to use a profile')
      );
    } catch (error) {
      console.error(chalk.red('✖ Error:'), error.message);
      process.exit(1);
  ...
  .action(async options => {
    try {
      ...
      if (validation.valid) {
        ...
        console.log(
          chalk.gray(
            `Thresholds: High(${config.thresholds.high_priority}) > Medium(${config.thresholds.medium_priority}) > Low(${config.thresholds.low_priority})`
          )
        );
      } else {
...
configCommand
  ...
  .argument(
    '<key>',
    'Configuration key (e.g., weights.impact, thresholds.high_priority)'
  )
  ...
  .action(async (key, value, options) => {
    try {
      ...
      if (!configPath) {
        ...
        console.log(
          chalk.yellow('Run `rubric init` to create a configuration file')
        );
        process.exit(1);
      }
      ...
      if (keyPath.length !== 2) {
        console.error(
          chalk.red(
            '✖ Error: Key must be in format section.property (e.g., weights.impact)'
          )
        );
        process.exit(1);
      }
